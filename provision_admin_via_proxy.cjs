...
async function provisionAdminViaProxy() {
  const adminEmail = 'admin@alnoor.sa';
  const adminPass = 'Password123!';

  console.log(`Provisioning ${adminEmail} via Edge Function Proxy...`);
  const createPayload = {
    action: 'create_user',
    email: adminEmail,
    password: adminPass,
  ...
  if (!createRes.ok || !createData.success) {
    console.error('Failed to provision admin via proxy:', createData.error);
    process.exit(1);
  }

  console.log(`[SUCCESS] Provisioned Super Admin in auth.users! User ID: ${createData.user.id}`);
