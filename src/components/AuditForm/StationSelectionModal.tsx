      ...
      <div className="bg-white/90 backdrop-blur-2xl border border-white/90 rounded-[28px] max-w-2xl w-full p-6 shadow-2xl ring-1 ring-white/60 animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-sky-100 shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-600 rounded-2xl text-white shadow-md shadow-sky-600/30">
              <Fuel className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-lg font-black text-slate-900">{t('stationSelect.modalTitle')}</h3>
              <p className="text-xs text-sky-700 font-extrabold">{t('stationSelect.modalSub')}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 mb-3 relative shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white border border-sky-200/80 rounded-2xl ps-10 pe-10 py-3 text-xs font-extrabold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/15 shadow-sm"
              autoFocus
            />
            {searchTerm && (
              <button
          ...
          <div className="flex items-center justify-between mt-2 px-1 text-[11px] text-slate-500 font-bold">
            <span>
            ...
            <span className="flex items-center gap-1 text-emerald-700 font-extrabold bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
        ...
        <div className="flex-1 overflow-y-auto my-2 pr-1 space-y-2.5 min-h-[220px] max-h-[380px]">
          {filteredStations.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-200 text-slate-500 text-xs font-medium italic">
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col justify-between relative group ${
                      isSelected
                        ? 'bg-gradient-to-br from-sky-50/90 via-white to-emerald-50/90 border-sky-500 shadow-lg shadow-sky-500/15 ring-2 ring-sky-500/30'
                        : 'bg-white/80 hover:bg-white border-slate-200/80 hover:border-sky-300 shadow-sm'
                    }`}
                  >
                    {/* TOP BADGE ROW */}
                    <div className="flex items-center justify-between mb-2">
                      <span
                        className={`text-[10px] font-black font-mono px-2.5 py-0.5 rounded-lg border ${
                          isSelected
                      ...
                      {isSelected ? (
                        ...
                        <div className="w-4 h-4 rounded-full border-2 border-slate-300 group-hover:border-sky-400" />
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {station.region && (
                        <div className="text-[10px] text-slate-400 font-semibold mt-0.5 ps-4">
                          {station.region}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-3 pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
                      <span className="text-slate-500 font-medium">{t('stationSelect.supervisor')}:</span>
        ...
        <div className="flex items-center justify-between border-t border-sky-100 pt-4 mt-2 shrink-0">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="px-6 py-2.5 bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white font-black text-xs rounded-2xl shadow-lg shadow-sky-600/30 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
          >
